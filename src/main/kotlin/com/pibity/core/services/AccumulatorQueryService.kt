...
import com.pibity.core.repositories.query.ValueAccumulatorRepository
import org.springframework.stereotype.Service
import java.sql.Timestamp

@Service
class AccumulatorQueryService(
    val userService: UserService,
    val typePermissionRepository: TypePermissionRepository,
    val variableQueryService: VariableQueryService,
    val valueAccumulatorRepository: ValueAccumulatorRepository
) {

  fun queryAccumulators(jsonParams: JsonObject, defaultTimestamp: Timestamp): Pair<List<VariableAccumulator>, TypePermission> {
    ...
    val typeAccumulator: TypeAccumulator = typePermission.type.uniqueConstraints.single { it.name == jsonParams.get(UniquenessConstants.UNIQUE_CONSTRAINT_NAME).asString }
      .accumulators.single { it.name == jsonParams.get(AccumulatorConstants.ACCUMULATOR_NAME).asString }
    val (generatedQuery, _, injectedValues) = try {
      generateAccumulatorQuery(queryJson = jsonParams.get(QueryConstants.QUERY).asJsonObject, username = jsonParams.get(OrganizationConstants.USERNAME).asString,
        typePermission = typePermission, typeAccumulator = typeAccumulator,
        defaultTimestamp = defaultTimestamp)
    } catch (exception: CustomJsonException) {
      throw CustomJsonException("{${QueryConstants.QUERY} : ${exception.message}}")
    }
    return Pair(valueAccumulatorRepository.queryAccumulators(hql = generatedQuery, injectedValues = injectedValues, limit = jsonParams.get(QueryConstants.LIMIT).asInt, offset = jsonParams.get(QueryConstants.OFFSET).asInt), typePermission)
  }

  fun generateAccumulatorQuery(queryJson: JsonObject, username: String, typePermission: TypePermission, typeAccumulator: TypeAccumulator, injectedValues: MutableMap<String, Any> = mutableMapOf(), defaultTimestamp: Timestamp): Triple<String, Int, MutableMap<String, Any>> {
    ...
    typePermission.keyPermissions
      .filter { keyPermission -> typeAccumulator.keys.any { it.name == keyPermission.key.name } }.forEach { keyPermission ->
        if (keyPermission.accessLevel > PermissionConstants.NO_ACCESS) {
          ...
          if (valuesJson.has(key.name) && key.type.name != TypeConstants.FORMULA) {
            ...
            var keyQuery = "SELECT $valueAlias FROM ValueAccumulator $valueAlias WHERE ${valueAlias}.variableAccumulator = $variableAlias AND ${valueAlias}.key = :v${variableCount}"
                    ...
                    keyQueryJson.has(QueryOperators.EQUALS) -> {
                      if (keyQueryJson.get(QueryOperators.EQUALS).isJsonObject) {
                        val (generatedQuery, count, _) = try {
                          variableQueryService.generateQuery(queryJson = keyQueryJson.get(QueryOperators.EQUALS).asJsonObject, injectedVariableCount = variableCount, injectedValues = injectedValues, parentValueAlias = valueAlias, username = username,
    ...
    return if (keyQueries.size != 0) {
      val hql = "SELECT DISTINCT $variableAlias FROM VariableAccumulator $variableAlias WHERE  ${variableAlias}.typeAccumulator = :v${variableCount} AND EXISTS " + keyQueries.joinToString(separator = " AND EXISTS ")
      injectedValues["v${variableCount++}"] = typeAccumulator
      Triple(hql, variableCount, injectedValues)
    } else {
      val hql = "SELECT DISTINCT $variableAlias  FROM VariableAccumulator $variableAlias  WHERE ${variableAlias}.typeAccumulator = :v${variableCount}"
      injectedValues["v${variableCount++}"] = typeAccumulator
      Triple(hql, variableCount, injectedValues)
    }
  }
