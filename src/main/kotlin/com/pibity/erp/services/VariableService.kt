...
class VariableService(
  ...
  fun createVariable(jsonParams: JsonObject, variableOrganization: Organization? = null, variableType: Type? = null, variableSuperList: VariableList? = null): Variable {
    ...
    val type: Type = variableType
        ?: typeRepository.findType(organization = organization, superTypeName = if (variableType != null) variableType.id.superTypeName else "Any", name = jsonParams.get("typeName").asString)
            ...
            if (value.id.key.type.id.superTypeName == "Any") {
              if (listValues.has("add")) {
                listValues.get("add").asJsonArray.forEach {
                  value.list!!.variables.add(
                      variableRepository.findVariable(organization = organization, superTypeName = value.id.key.type.id.superTypeName, typeName = value.id.key.type.id.name, superList = organization.superList!!.id, name = it.asString)
              ...
              if ((value.id.key.id.parentType.id.superTypeName == "Any" && value.id.key.id.parentType.id.name == value.id.key.list!!.type.id.superTypeName)
                  ...
                  listValues.get("add").asJsonArray.forEach {
                    value.list!!.variables.add(
                        try {
                          ...
                          throw CustomJsonException("{${value.id.key.id.name}: {${it.asString}: {add: 'Unable to add variable to list'}}}")
                        }
                    )
                  }
                }
                if (listValues.has("remove")) {
                  listValues.get("remove").asJsonArray.forEach {
                    val variableToDelete = variableRepository.findVariable(organization = organization, superTypeName = value.id.key.type.id.superTypeName, typeName = value.id.key.type.id.name, superList = value.list!!.id, name = it.asString)
                            ?: throw CustomJsonException("{${value.id.key.id.name}: {remove: 'Unable to find ${it.asString} referenced in list'}}")
                    value.list!!.variables.remove(variableToDelete)
                    variableRepository.delete(variableToDelete)
                  }
                }
                if (listValues.has("update")) {
                  listValues.get("update").asJsonArray.forEach {
                    val variableToUpdate: Variable = variableRepository.findVariable(organization = organization, superTypeName = value.id.key.type.id.superTypeName, typeName = value.id.key.type.id.name, superList = value.list!!.id, name = it.asString)
                ...
                if (listValues.has("add")) {
                  listValues.get("add").asJsonArray.forEach {
                    value.list!!.variables.add(
                        variableRepository.findVariable(organization = organization, superTypeName = value.id.key.type.id.superTypeName, typeName = value.id.key.type.id.name, superList = it.asJsonObject.get("context").asLong, name = it.asJsonObject.get("variableName").asString)
                            ?: throw CustomJsonException("{${value.id.key.id.name}: {add: 'Unable to insert ${it.asString} referenced in list'}}")
                    )
                  }
                }
