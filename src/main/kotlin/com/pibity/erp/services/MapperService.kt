...
class MapperService(
  ...
  fun createMapper(jsonParams: JsonObject): Mapper {
    ...
    return mapperJpaRepository.save(Mapper(organization = functionInput.function.organization, name = validateMapperName(jsonParams.get(MapperConstants.MAPPER_NAME).asString), query = jsonParams.get(MapperConstants.QUERY).asBoolean, functionInput = functionInput).apply {
      if (query)
        queryParams.apply {
  ...
  fun executeMapper(jsonParams: JsonObject, defaultTimestamp: Timestamp = Timestamp(System.currentTimeMillis()), files: MutableList<MultipartFile>): JsonArray {
    ...
    return if (mapper.query) {
      ...
      val variables: List<Variable> = queryService.queryVariables(jsonParams = JsonObject().apply {
