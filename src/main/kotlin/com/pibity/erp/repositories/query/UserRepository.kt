...
class UserRepository(val entityManager: EntityManager) {
  ...
  @Transactional(readOnly = true)
  fun getUserTypePermissions(orgId: Long, typeName: String, username: String): Set<TypePermission> {
    val hql = "SELECT DISTINCT p0 FROM TypePermission p0 WHERE EXISTS (SELECT p FROM TypePermission p JOIN p.permissionRoles pr JOIN pr.id.role r JOIN r.roleUsers ru JOIN ru.id.user u WHERE p.type.organization.id = :orgId AND p.type.name = :typeName AND u.organization.id = :orgId AND u.username = :username AND p = p0) OR EXISTS (SELECT p FROM TypePermission p JOIN p.permissionRoles pr JOIN pr.id.role r JOIN r.roleGroups rg JOIN rg.id.group g JOIN g.groupUsers gu JOIN gu.id.user u WHERE p.type.organization.id = :orgId AND p.type.name = :typeName AND u.organization.id = :orgId AND u.username = :username AND p = p0)"
  ...
  @Transactional(readOnly = true)
  fun getUserFunctionPermissions(orgId: Long, functionName: String, username: String): Set<FunctionPermission> {
    val hql = "SELECT DISTINCT p0 FROM FunctionPermission p0 WHERE EXISTS (SELECT p FROM FunctionPermission p JOIN p.permissionRoles pr JOIN pr.id.role r JOIN r.roleUsers ru JOIN ru.id.user u WHERE p.function.organization.id = :orgId AND p.function.name = :functionName AND u.organization.id = :orgId AND u.username = :username AND p = p0) OR EXISTS (SELECT p FROM FunctionPermission p JOIN p.permissionRoles pr JOIN pr.id.role r JOIN r.roleGroups rg JOIN rg.id.group g JOIN g.groupUsers gu JOIN gu.id.user u WHERE p.function.organization.id = :orgId AND p.function.name = :functionName AND u.organization.id = :orgId AND u.username = :username AND p = p0)"
