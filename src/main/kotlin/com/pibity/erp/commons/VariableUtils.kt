                    ...
                    values.get(key.id.name).asJsonObject.get("add").asJsonArray.toSet().forEach {
                      try {
                        ...
                        if (!it.asJsonObject.has("values"))
                          ...
                          try {
                            jsonObject.add("values", validateVariableValues(it.asJsonObject.get("values").asJsonObject, key.type))
                    ...
                    values.get(key.id.name).asJsonObject.get("update").asJsonArray.toSet().forEach {
                      try {
                        ...
                        if (!it.asJsonObject.has("values"))
                          ...
                          try {
                            jsonObject.add("values", validateUpdatedVariableValues(it.asJsonObject.get("values").asJsonObject, key.type))
                          } catch (exception: CustomJsonException) {
